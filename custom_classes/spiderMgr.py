...
class SpiderMgr(QObject):  
    ...
    def addTask(self,user_url):
        ...
        if not res:
            print('链接解析失败')
            return False
        user_agent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36 Edg/121.0.0.0 AtContent/95.5.5392.49'
        headers = {
    ...
    def down_vedio(self,nikename,titles,vedio_addrs,indexs,user_url):
        headers = {
        ...
        for index in indexs:
            ...
            resp = requests.get(url=url,headers=headers,proxies=self.proxies)
            with open(f'{filepath}\\{titles[index]}.mp4',mode='wb') as f:
                f.write(resp.content)
            print(f'{titles[index]}.mp4下载完成')