...
parmas = {
    ...
    # 'sec_user_id':sec_user_id, #作者本身的表示
    'count':'18', #请求的数量。
...
#https://www.douyin.com/user/MS4wLjABAAAAD5lgLAKcy657lsJ3N_jT62xJsnTtbcjimwvRq_yr2OoKqqgt0vQFZGJ2sgcd3rnM?from_tab_name=main&modal_id=7008414685766749447
def get_media_url(user_url):
    sec_user_id = user_url.split('?',1)[0].rsplit('/',1)[1]
    parmas['sec_user_id']=sec_user_id
    # print(sec_user_id)
    resp = requests.get(url=url,headers=headers,proxies=proxies,params=parmas,stream=True)
    resp.encoding = 'utf-8'
    # print(resp.text)
    obj = re.compile('"desc":"(?P<title>.*?)","',re.S)
    its = obj.finditer(resp.text)
    for it in its:
        if(it.group('title')[0] == '1'):
            continue
